...
typedef struct alsa_seq_info {
	...
	short                       auto_hw;
	short                       auto_sw;
	ALSA_SEQ_PORT               *rx_port;
	ALSA_SEQ_PORT               *tx_port;
	ALSA_SEQ_PORT               *src_ports;
	ALSA_SEQ_PORT               *dest_ports;
...
int alsa_seq_port_list_compare(ALSA_SEQ_PORT *a,
                               ALSA_SEQ_PORT *b);
ALSA_SEQ_PORT *alsa_seq_get_port_list(ALSA_SEQ_INFO *midi,
                                      unsigned int caps,
                                      ALSA_SEQ_PORT *orig_list);
void alsa_seq_subscribe_port(ALSA_SEQ_INFO *midi,
                             ALSA_SEQ_PORT *seq_port,
                             char *port_str_list,
                             int tx);
void alsa_seq_unsubscribe_port(ALSA_SEQ_INFO *midi,
                               ALSA_SEQ_PORT *seq_port);
...
ALSA_SEQ_PORT *get_alsa_seq_ports(ALSA_SEQ_INFO *seq_info,
                                  char *alsa_ports);
ALSA_SEQ_INFO *open_alsa_seq_in(char *alsa_capture_ports,
                                char *alsa_playback_ports);
void alsa_seq_cleanup(void *arg);
int alsa_seq_init(void);
void *alsa_seq_rx_thread(void *UNUSED(arg));
void *alsa_seq_tx_thread(void *UNUSED(arg));
